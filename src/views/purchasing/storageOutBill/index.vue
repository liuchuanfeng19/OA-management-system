...
// const inStockStatusNV = ref([]); // 出库状态列表
...
function handleExport() {
  const fileName = "出库单";
  exportExcel("/api/OutStock/Export?", fileName, queryForm);
}
...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await deleteOutStock({ id: row.id });
      if (code == 0) {
...
<template>
  <div class="main">
    ...
    >
      ...
      >
        <el-input
          v-model.trim="queryForm.outStockCode"
          placeholder="请输入"
          clearable
        />
      </el-form-item>
      ...
      <template v-slot="{ size, checkList }">
        ...
        >
          ...
          <el-table-column
            ...
            :formatter="
              ({ outStockTime }) =>
                outStockTime ? dayjs(outStockTime).format('YYYY-MM-DD') : '-'
            "
            align="center"
          />
          <el-table-column
            ...
            :formatter="
              ({ auditTime }) =>
                auditTime ? dayjs(auditTime).format('YYYY-MM-DD') : '-'
            "
            align="center"
          />
          <el-table-column
