...
import { DeletePurJoinSign, GetListPurJoinSign } from "@/api/purJoinSign";
...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    const { code, message } = await DeletePurJoinSign({ id: row.id });
    if (code == 0) {
...
async function onSearch() {
  loading.value = true;
  GetListPurJoinSign(queryForm.value)
    .then(({ data }) => {
    ...
    >
      ...
      <template v-slot="{ size, checkList }">
        ...
        >
          ...
          <el-table-column align="center" label="采购明细" min-width="100">
            <template #default="{ row }">
          ...
          >
            <template #default="{ row }">
              <div v-if="row.attachSupply && row.attachSupply.length > 0">
                <PreviewButton :srcList="row.attachSupply" :size="size" />
                <DownloadButton :srcList="row.attachSupply" :size="size" />
              </div>
            </template>
          </el-table-column>
