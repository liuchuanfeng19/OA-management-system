...
import {
  getNonDailyExpenseList,
  deleteNonDailyExpense
...
import { getInvoiceCategories, getExpenseCategories } from "@/api/dailyExpense";
...
function onSearch() {
  ...
  getNonDailyExpenseList(queryForm)
    .then(({ data }) => {
...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await deleteNonDailyExpense({
        id: row.id
      });
      if (code == 0) {
...
function handleExport() {
  const fileName = "非日常费用报销";
  exportExcel("/api/NonDailyExpense/Export?", fileName, queryForm);
}
    ...
    >
      ...
      <template v-slot="{ size, checkList }">
        ...
        >
          ...
          <el-table-column v-if="false" label="发票" align="center" width="100">
            <template #default="{ row }">
              <div v-if="row.invoiceAttachList">
                <PreviewButton :srcList="row.invoiceAttachList" :size="size" />
                <DownloadButton :srcList="row.invoiceAttachList" :size="size" />
              </div>
            </template>
          </el-table-column>
