...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await deleteNoticeRule({ id: row.id });
      if (code == 0) {
  ...
  <div class="main">
    ...
    >
      <el-form-item
        ...
        prop="isEnable"
      >
        <el-select v-model="queryForm.isEnable" clearable>
      ...
      <el-form-item
        ...
        prop="bigType"
      >
        <el-select v-model="queryForm.bigType" clearable>
          <el-option
            v-for="item in allBusinessType"
            :key="item.value"
            :label="item.name"
            :value="item.value"
          />
