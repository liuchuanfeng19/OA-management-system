...
import { getStaffList, ConfirmStaffOff } from "@/api/personnel";
...
// 表单校验规则
const rules = {
  offRealDate: [{ required: true, message: "请输入离职日期", trigger: "blur" }]
};
...
watch(
  ...
  val => {
    ...
    //日期缺省
    // formData.value.offRealDate = new Date(+new Date() + 8 * 3600 * 1000)
    //   .toISOString()
    //   .replace(/T/g, " ")
    //   .replace(/\.[\d]{3}Z/, "");
    formData.value.offRealDate = moment().format("YYYY/MM/DD");
  }
);
...
const submitForm = (formEl: FormInstance | undefined, flag: boolean) => {
  ...
  formEl.validate(async valid => {
    if (valid) {
      const { code, message } = await ConfirmStaffOff(data);
      if (code == 0) {
    ...
    >
      <el-row :gutter="20">
        ...
        <el-col :span="24" :offset="0">
          <el-form-item label="离职备注" prop="offComment">
            <el-input
              v-model="formData.offComment"
              type="textarea"
              placeholder="请输入"
              maxlength="256"
              show-word-limit
