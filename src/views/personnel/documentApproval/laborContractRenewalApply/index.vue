...
defineOptions({
  name: "LaborContractRenewalApply"
});
...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await DeleteStaffRenew({ id: row.id });
      if (code == 0) {
...
async function handleHidden(row) {
  ElMessageBox.confirm("确定要隐藏吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await staffRenewmarkAsNotDisplay({
        id: row.id
      });
      if (code == 0) {
        ElMessage.success(message);
...
async function handleExportOne(row) {
  ...
  const fileName = "签订劳动合同审批表";
  exportExcel("/api/StaffRenew/ExportOne?", fileName, id.value);
}
...
async function onSearch() {
  loading.value = true;
  const { data } = await GetListStaffRenew(queryForm.value);
