...
import { decmarkAsNotDisplay } from "@/api/decasualization";
import {
  DeleteStaffOffJoinSign,
  GetListStaffOffJoinSign
...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await DeleteStaffOffJoinSign({ id: row.id });
      if (code == 0) {
...
async function handleExportOne(row) {
  ...
  const fileName = "离职会签单";
  exportExcel("/api/StaffOffJoinSign/ExportOne?", fileName, id.value);
}
...
async function onSearch() {
  loading.value = true;
  const { data } = await GetListStaffOffJoinSign(queryForm.value);
  ...
  _data.forEach(item => {
    ...
    //导出
    item.disabled["handleExportOne"] = !item.canExport;
    item.title["handleExportOne"] = "";
  });
...
<template>
  <div class="main">
    ...
    >
      ...
      >
        <el-select clearable v-model="queryForm.AyPostId" placeholder="请选择"
          ><el-option
