...
import { getStatisticsList, attendAccRecordStatistics } from "@/api/attendance";
...
const getStaffListByillegal = async () => {
  const { data = {} } = await getStaffList({ pageIndex: 1, pageSize: 999 });
  userList.value = data.data || [];
};
function handleStaffChange(val) {
  userList.value.forEach(item => {
    if (val == item.staffName) {
...
async function onSearch() {
  loading.value = true;
  const { data } = await getStatisticsList(queryForm.value);
  dataList.value = data.data || [];
  if (dataList.value.length > 0) {
    const item = dataList.value[0];
    columnList.value = item.leaveItems || [];
  }
...
async function handleStatistics() {
  ...
  if (queryForm.value.month == null || queryForm.value.month == "") {
    ElMessage.warning("请选择月份");
    return;
  }
  const param = {
    staffId: staffId.value,
    monthDt: queryForm.value.month
  };
  const { data } = await attendAccRecordStatistics(param);
  if (data) {
...
async function handleExport() {
  exportLoading.value = true;
  const fileName = "考勤统计";
  await exportExcel("/api/AttendAccRecord/Export?", fileName, queryForm.value);
  exportLoading.value = false;
}
...
<template>
  <div class="main">
    ...
    >
      ...
      <!-- <el-form-item label="员工姓名" prop="staffName" v-show="maxItemNum >= 1">
        <el-input
          placeholder="请输入"
          v-model="queryForm.staffName"
          clearable
    ...
    >
      <template #buttons>
        <el-button
          v-auth="'AttendanceStatistics.export'"
          type="primary"
          :icon="useRenderIcon('histogram')"
      ...
      <template v-slot="{ size, checkList }">
        ...
        >
          ...
          <el-table-column
            ...
            min-width="100"
            align="center"
            prop="attStaffStartDate"
            show-overflow-tooltip
            :formatter="
              ({ attStaffStartDate }) => {
                return dayjs(attStaffStartDate).format('YYYY-MM-DD');
              }
            "
          />
          <el-table-column
          ...
          >
            <el-table-column
              ...
              width="100"
              align="center"
              prop="attStaffEndDate"
              show-overflow-tooltip
              :formatter="
                ({ attStaffEndDate }) => {
                  return attStaffEndDate != null && attStaffEndDate != ''
                    ? dayjs(attStaffEndDate).format('YYYY-MM-DD')
                    : '';
                }
              "
            />
            <el-table-column
              ...
              width="100"
              align="center"
              prop="attStaffEndMonth"
              show-overflow-tooltip
              :formatter="
                ({ attStaffEndMonth }) => {
                  return attStaffEndMonth != null && attStaffEndMonth != ''
                    ? dayjs(attStaffEndMonth).format('YYYY-MM')
                    : '';
                }
              "
            />
          ...
          <!-- <el-table-column fixed="right" label="操作" width="80" align="center">
            <template #default="scope">
              <el-button
                v-auth="'AttendanceStatistics.query'"
                :size="size"
