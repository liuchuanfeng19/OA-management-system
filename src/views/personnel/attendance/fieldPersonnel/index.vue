...
import { getOutAttendApplyPage } from "@/api/fieldPersonnel";
...
async function handleExport() {
  const fileName = "外勤";
  exportExcel("/api/OutAttendApply/Export?", fileName, queryForm.value);
}
...
async function onSearch() {
  loading.value = true;
  getOutAttendApplyPage(queryForm.value)
    .then(({ data }) => {
...
async function getApplyStatus() {
  const { data } = await GetCommonApplyStatus1({ includeAll: true });
  ...
  <div class="main">
    ...
    >
      ...
      <el-form-item
        ...
        prop="applyStatus"
      >
        <el-select v-model="queryForm.applyStatus" placeholder="请选择">
