...
import {
  getListWithSiteForDrawing,
  GetForDrawing
} from "@/api/sellContractItem";
...
function onSearch() {
  ...
  getListWithSiteForDrawing(queryForm)
    .then(({ data }) => {
...
function handleExport() {
  const fileName = "xx导出";
  exportExcel("/api/controller/Export?", fileName, queryForm);
}
        ...
        >
          ...
          >
            <template #default="{ row }">
              <span>{{ row.items[idx].drawingQty }}</span>
            </template>
          </el-table-column>
          <TableColumOperation
