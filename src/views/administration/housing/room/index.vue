...
const allBuildsRoomStatus = ref([]); //房间状态
...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await DeleteRoom({ id: row.id });
      if (code == 0) {
...
//获取所有房间状态
async function getBuildsRoomStatus() {
  const { data } = await GetBuildsRoomStatusNV({ includeAll: false });
  allBuildsRoomStatus.value = data;
}
//加载所有房屋列表;
getBuildsListTypes();
//加载所有房间状态;
    ...
    >
      ...
      >
        <el-select v-model="queryForm.status" clearable placeholder="请选择"
          ><el-option
            v-for="item in allBuildsRoomStatus"
            :key="item.value"
            :label="item.name"
            :value="item.value"
        /></el-select>
