...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await OfficeObjectDelete({ id: row.id });
      if (code == 0) {
...
function handleExport() {
  const fileName = "办公用品列表";
  exportExcel("/api/OfficeObject/Export?", fileName, queryForm);
}
...
<template>
  <div class="main">
    ...
    >
      ...
      >
        <el-input
          v-model.trim="queryForm.assetsName"
          placeholder="请输入"
          clearable
        />
      </el-form-item>
