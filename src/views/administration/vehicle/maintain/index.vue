...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await DeleteMaintain({ id: row.id });
      if (code == 0) {
...
async function onSearch() {
  loading.value = true;
  GetListByCarInformationId(queryForm.value)
    .then(({ data }) => {
...
//车牌模糊查询
async function CarInformation() {
  loading.value = true;
  const { data } = await GetCarLicensePlatePattern(Form.value);
  Carist.value = data.data || [];
}
CarInformation();
  ...
  <div class="main">
    ...
    >
      ...
      <!-- <el-form-item label="车辆" prop="carInformationId">
        <el-select v-model="queryForm.carInformationId" placeholder="请选择车辆"
          ><el-option
    ...
    <ReadDialog ref="readDialogRef" :title="title" :width="676" :column="2" />
  </div>
</template>
