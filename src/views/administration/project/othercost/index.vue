...
import { GetListCostOther, DeleteCostOther, GetListNV } from "@/api/project";
...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await DeleteCostOther({ id: row.id });
      if (code == 0) {
...
async function onSearch() {
  loading.value = true;
  const { data } = await GetListCostOther(queryForm.value);
    ...
    >
      ...
      <template v-slot="{ size, checkList }">
        ...
        >
          ...
          <el-table-column
            v-auth="'OtherCost.query|OtherCost.edit|OtherCost.delete'"
            fixed="right"
            label="操作"
            width="160"
            align="center"
