...
import { GetListbudget, Deletebudget, GetListNV } from "@/api/project";
...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await Deletebudget({ id: row.id });
      if (code == 0) {
...
async function onSearch() {
  loading.value = true;
  const { data } = await GetListbudget(queryForm.value);
    ...
    >
      ...
      <template v-slot="{ size, checkList }">
        ...
        >
          ...
          <el-table-column
            v-auth="
              'ProjectBudget.query|ProjectBudget.edit|ProjectBudget.delete'
            "
            fixed="right"
            label="操作"
            width="160"
