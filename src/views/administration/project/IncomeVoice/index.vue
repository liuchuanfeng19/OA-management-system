...
import { DeleteInvoice, GetInvoiceList } from "@/api/project";
...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await DeleteInvoice({ id: row.id });
      if (code == 0) {
...
async function onSearch() {
  loading.value = true;
  const { data } = await GetInvoiceList(queryForm.value);
    ...
    >
      ...
      <template v-slot="{ size, checkList }">
        ...
        >
          ...
          <el-table-column
            v-auth="
              'ProjIncVoice.query|ProjIncVoice.edit|ProjIncVoice.audi|ProjIncVoice.delete'
            "
            fixed="right"
            label="操作"
            width="200"
