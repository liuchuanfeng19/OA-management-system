...
const rules = {
  ...
  projBudgetId: [{ required: true, message: "请选择月度", trigger: "blur" }],
  name: [{ required: true, message: "请选择费用名称", trigger: "blur" }],
  total: [{ required: true, message: "请输入费用总额", trigger: "blur" }]
};

// 修改表单时根据id获取详情数据
async function getDetail(id) {
  formLoading.value = true;
  const { data } = await GetProChildbudget({ id });
  formData.value = data || {};
  ...
  formEl.validate(async valid => {
    if (valid) {
      if (formData.value.id) {
        const { message, data } = await UpdateChildbudget(formData.value);
        if (data) {
        ...
        const { message, data } = await CreateChildbudget(formData.value);
        if (data) {
          ElMessage.success(message);
          formVisible.value = false;
          resetForm(formEl);
