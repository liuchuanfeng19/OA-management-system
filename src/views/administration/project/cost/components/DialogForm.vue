...
const rules = {
  ...
  staffCost: [{ required: true, message: "请输入人员成本", trigger: "blur" }],
  otherCost: [{ required: true, message: "请输入其他成本", trigger: "blur" }],
  projIncome: [
...
async function getDetail(id) {
  ...
  const { data } = await GetProjCost({ id });
  formData.value = data || {};
  ...
  formEl.validate(async valid => {
    if (valid) {
      if (formData.value.id) {
        const { message, data } = await UpdateProjCost(formData.value);
        if (data) {
        ...
        const { message, data } = await CreateProjCost(formData.value);
        if (data) {
          ElMessage.success(message);
          formVisible.value = false;
          resetForm(formEl);
