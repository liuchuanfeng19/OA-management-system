...
import { GetListProjCost, DeleteProjCost, GetListNV } from "@/api/project";
...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await DeleteProjCost({ id: row.id });
      if (code == 0) {
...
async function onSearch() {
  loading.value = true;
  const { data } = await GetListProjCost(queryForm.value);
    ...
    >
      ...
      <template v-slot="{ size, checkList }">
        ...
        >
          ...
          <el-table-column
            v-auth="'ProjectCost.query|ProjectCost.edit|ProjectCost.delete'"
            fixed="right"
            label="操作"
            width="160"
            align="center"
