...
import { GetFinanceList, DeleteFinance, GetProjStatusNV } from "@/api/project";
...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await DeleteFinance({ id: row.id });
      if (code == 0) {
...
async function onSearch() {
  loading.value = true;
  const { data } = await GetFinanceList(queryForm.value);
    ...
    >
      ...
      <template v-slot="{ size, checkList }">
        ...
        >
          ...
          <el-table-column
            label="实际收款时间"
            prop="realReceiveTime"
            width="180"
            align="center"
          >
            <template #default="{ row }">
              {{ formatDatetime(row.realReceiveTime) }}
            </template>
          </el-table-column>
