...
import { StaffbudgetList, DeleteStaffbudget, GetListNV } from "@/api/project";
...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await DeleteStaffbudget({ id: row.id });
      if (code == 0) {
...
async function onSearch() {
  loading.value = true;
  const { data } = await StaffbudgetList(queryForm.value);
    ...
    >
      ...
      <template v-slot="{ size, checkList }">
        ...
        >
          ...
          <el-table-column
            v-auth="
              'ProjectChildBudget.query|ProjectChildBudget.edit|ProjectChildBudget.delete'
            "
            fixed="right"
            label="操作"
            width="160"
          ...
          >
            <template #default="scope">
              <el-button
                v-auth="'ProjectChildBudget.query'"
                :size="size"
                link
                type="primary"
              ...
              <el-button
                v-auth="'ProjectChildBudget.edit'"
                :size="size"
                link
                type="primary"
              ...
              <el-button
                v-auth="'ProjectChildBudget.delete'"
                :size="size"
                link
                type="primary"
