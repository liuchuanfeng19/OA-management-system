...
const _buyMethodList = ref([]);
const _noticeTypeList = ref([]);
...
watch(
  ...
  newValue => {
    if (newValue && newValue.length > 0) {
      INITIAL_DATA.buyMethod = newValue[0].value;
      INITIAL_DATA.buyMethodName = newValue[0].name;
      _buyMethodList.value = newValue;
    }
  }
);
watch(
  ...
  newValue => {
    if (newValue && newValue.length > 0) {
      INITIAL_DATA.noticeType = newValue[0].value;
      INITIAL_DATA.noticeTypeName = newValue[0].name;
      _noticeTypeList.value = newValue;
    }
  }
);
...
const show = async (_formData, _type) => {
  ...
  if (_buyMethodList.value.length == 0) {
    getBuyMethod();
  }
  if (_noticeTypeList.value.length == 0) {
    getNoticeType();
  }
  formVisible.value = true;
  showType.value = _type;
  formData.value = { ...INITIAL_DATA };
  // formData.value.busiStaffName = staffName;
  ...
  formEl.validate(async valid => {
    if (valid) {
      ...
      if (showType.value == "add") {
        formData.value.inStatus = 1;
        formData.value.biddingStatus = 1;
        CreateBidding({ ...params })
          .then(({ code, message }) => {
            if (code == 0) {
  ...
  >
    ...
    >
      <el-row :gutter="20">
        ...
        <el-col :span="24" :offset="0">
          <el-form-item label="招标单位" prop="unit">
            <el-input
        ...
        <el-col
          ...
          :offset="0"
        >
          <el-form-item label="参与状态" prop="inStatusName">
            <el-input v-model.trim="formData.inStatusName" readonly />
          </el-form-item>
        </el-col>
        <el-col :span="12" :offset="0">
          <el-form-item label="商务专员" prop="busiStaffId">
            <!-- <el-input v-model="formData.busiStaffName" readonly /> -->
            <el-select
