...
const invoiceCategoryList = ref([]); // 发票种类列表数据
...
function onSearch() {
  ...
  getProjectList();
  getInvoiceCompanyList(queryForm)
    .then(({ data }) => {
      const _data = data.data || [];
      tableData.value = _data.map(item => {
        item.planCollectTime = item.planCollectTime
          ? moment(item.planCollectTime).format("YYYY-MM-DD")
          : "-";
        item.disabled = {};
        item.title = {};
        //编辑
...
function getInvoiceCategoryList() {
  getConfigNameValueListByKey({
    ...
    .then(({ data }) => {
      invoiceCategoryList.value = data || [];
    })
    .catch(() => {
      invoiceCategoryList.value = [];
    });
}
// 获取发票种类列表数据
...
async function handleDelete(row) {
  ElMessageBox.confirm("确定要删除吗?", "提示", {
    ...
    .then(async () => {
      const { code, message } = await deleteInvoiceCompany({ id: row.id });
      if (code == 0) {
...
<template>
  <div class="main">
    ...
    <FormDialog
      ...
      @search="onSearch"
    />
    <ReadDialog ref="readDialogRef" :title="title" :width="652" :column="2" />
  </div>
</template>
