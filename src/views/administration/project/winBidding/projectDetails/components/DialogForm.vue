...
import { GetHistoryRecvAccList } from "@/api/joinSignItem";
...
const dialogTitle = computed(() => {
  return type.value == "details" ? "发货时间明细" : "";
});
...
async function getDetail(isItem, historyId) {
  ...
  const { data } = await GetHistoryRecvAccList({
        ...
        >
          ...
          >
            ...
            >
              <template #default="{ row }">
                <span>{{
                  row.siteItems[idx].realReceiveTime
                    ? formatDatetime(row.siteItems[idx].realReceiveTime)
                    : "-"
                }}</span>
              </template>
            </el-table-column>
