  ...
  function concatPath(arr: object[], value: string, parentPath: string) {
    if (!hasValue) {
      arr.forEach((arrItem: any) => {
        ...
        if (
          (arrItem.path === value || pathConcat === value) &&
          arrItem.meta.showLink
        ) {
...
function deleteDynamicTag(
  ...
  if (current === route.path) {
    ...
    if (tag === "left") return;
    if (targetPath) {
      const targetIndex: number = multiTags.value.findIndex(
        (item: any) => item.path === targetPath
      );
      ...
      if (targetIndex == -1) {
        router.push({
          path: obj.parentPath,
          query: obj.query as any
        });
        return;
      }
      nextTick(() => {
        router.push({
          path: multiTags.value[targetIndex].path,
          query: multiTags.value[targetIndex].query as any
        });
      });
      return;
    }
...
onBeforeMount(() => {
  ...
  //  接收页面返回按钮事件
  emitter.on("closeTag", _route => {
    console.log("close _route", _route);
    console.log("close multiTags", multiTags.value);
    const currentTag = multiTags.value.find(
      item =>
        item.name == _route.routeName &&
        (JSON.stringify(item.query) === JSON.stringify(_route.query) ||
          (JSON.stringify(_route.query) == "{}" &&
            (item.query == undefined || typeof item.query == "undefined")))
    );
    console.log("currentTag", currentTag);
    deleteDynamicTag(currentTag, currentTag.path, "", _route.targetPath);
  });
});
